...
// Watson IoT connection details--Xingyu's mkr1010
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (0 == strcmp(topic, MQTT_TOPIC_alarm)) {
    ...
    if(alarm_value == 1){ // Green Light
      ...
      hospital();
    } else if(alarm_value == 4){ // Red light + buzzer
      patient();
    }
...
// --------------------------------------------------------------------------------------------
//         Define the actuators' actions according to alarm types 
// --------------------------------------------------------------------------------------------
...
// alarm type 2: the patient has some abnormal situations > threshold
// 室友那里灯黄色 + 蜂鸣器
// 医院：email-->看dashboard
void abnormal()
{
...
// alarm type 3: Hospital seeks roommates to check on patients
...
// alarm type 4: the patient asks for help
